...
const Renderer = (() => {
  ...
  function getTileColor(tile){
    return {normal:CONFIG.COLORS.TILE_NORMAL,high:CONFIG.COLORS.TILE_HIGH,obstacle:CONFIG.COLORS.TILE_OBSTACLE,water:CONFIG.COLORS.TILE_WATER,hazard:CONFIG.COLORS.TILE_HAZARD}[tile.terrain]||CONFIG.COLORS.TILE_NORMAL;
  }
  ...
  function drawTile(tile){
    ...
    switch (tile.terrain) {
      ...
      case 'hazard':   drawTerrainHazard(ctx, tile, px, py, ts);   break;
      default:
  ...
  function drawUnit(unit){
    ...
    if(unit.statusEffects&&unit.statusEffects.length>0){
      const SI={poison:'☠',stun:'★',slow:'⧖',shield:'◉',rooted:'❧',suppressed:')',rallied:'!',hidden:'◎',ley_spent:'◈'};
      const seFs = Math.max(7, Math.floor(ts * 0.17));
      unit.statusEffects.forEach((se,i)=>{
        ctx.font=`${seFs}px serif`;
        ctx.textAlign='left'; ctx.textBaseline='top';
        ctx.fillStyle=se.name==='poison'?'#a0ff60':se.name==='stun'?'#ffee00':se.name==='rallied'?'#ffd740':se.name==='hidden'?'#aaaaaa':'#80c8ff';
